...
parts = [
    [
        [int(num) for num in dashed_string.split("-")]
        for dashed_string in line.split(",")
    ]
    for line in lines
]
...
p1 = 0
for part in parts:
    if (part[0][0] <= part[1][0] and part[0][1] >= part[1][1]) or (
        part[1][0] <= part[0][0] and part[1][1] >= part[0][1]
    ):
        p1 += 1
...
for part in parts:
    ...
    if (
        len(
            set(range(part[0][0], part[0][1] + 1)).intersection(
                set(range(part[1][0], part[1][1] + 1))
            )
        )
        > 0
    ):
